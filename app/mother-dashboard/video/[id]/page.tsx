...
export default async function MotherVideoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const activeModuleIndex = lesson.modules.findIndex((module) => module.id === lesson.video.module.id);
  const sidebarModules = [
    lesson.modules[activeModuleIndex],
    activeModuleIndex >= 0 ? lesson.modules[activeModuleIndex + 1] : undefined,
  ].filter((module): module is (typeof lesson.modules)[number] => Boolean(module));
  ...
  return (
    <PageShell>
      <Section tone="beige" eyebrow="Lesson player" title="Course lesson" text="Watch the lesson, mark your progress, and continue when you are ready.">
          ...
          <div className="grid content-start gap-5">
            ...
            <div className="rounded-soft bg-white p-5 shadow-soft ring-1 ring-borderSoft">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-3 max-w-3xl text-sm leading-6 text-ink/68">
                    {lesson.video.description ?? "Lesson notes, teaching points, and practice reminders can be added here."}
                  </p>
                </div>
              ...
              <div className="mt-5 grid gap-3 border-t border-borderSoft pt-4 md:grid-cols-2">
                <div className="rounded-2xl bg-beige/60 p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-ink/68">Revise this lesson gently and mark it complete after watching.</p>
                </div>
                <div className="rounded-2xl bg-beige/60 p-4">
                  <div className="flex items-center gap-2 text-sm font-bold text-blueDeep">
                  ...
                  {resources.length === 0 ? <p>PDFs and worksheets will appear here when uploaded.</p> : null}
                  {resources.map((resource) => {
                    const resourceUnlocked = canAccess(resource.required_access, accessLevel);
                    return (
                      <a key={resource.id} href={resourceUnlocked ? resource.file_url : "/mother/dashboard#payment-status"} className={`rounded-xl bg-white px-3 py-2 text-xs font-bold ${resourceUnlocked ? "text-blueDeep" : "text-ink/45"}`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              {lesson.previousVideo ? (
                <Link href={`/mother-dashboard/video/${lesson.previousVideo.id}`} className="focus-ring inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-blueDeep shadow-sm ring-1 ring-borderSoft hover:bg-beige">
                  <ArrowLeft className="h-4 w-4" />
                  Previous lesson
                </Link>
              ) : <span />}
              {lesson.nextVideo && canAccess(lesson.nextVideo.required_access, accessLevel) ? (
                <Link href={`/mother-dashboard/video/${lesson.nextVideo.id}`} className="focus-ring inline-flex items-center gap-2 rounded-full bg-blueDeep px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[#102A56]">
                  Next lesson
            ...
            <div className="max-h-[calc(100vh-9rem)] overflow-y-auto rounded-soft bg-white p-5 shadow-soft ring-1 ring-borderSoft">
              ...
              <div className="mt-4 grid gap-3">
                {sidebarModules.map((module) => {
                  ...
                  return (
                  <details key={module.id} open={isActiveModule} className={`rounded-2xl p-3 ${isActiveModule ? "bg-beige/70 ring-1 ring-coral/20" : "bg-white ring-1 ring-borderSoft"}`}>
                    <summary className="cursor-pointer list-none text-sm font-bold leading-5 text-blueDeep">
                      {module.title}
                      <span className="mt-1 block text-xs font-semibold text-ink/55">{module.videos.length} lessons</span>
                    </summary>
                ...
                <Link href="/mother/dashboard" className="focus-ring rounded-full border border-blueDeep/15 bg-white px-4 py-3 text-center text-sm font-bold text-blueDeep shadow-sm transition hover:bg-beige">
                  Browse all modules
                </Link>
              </div>
            </div>
