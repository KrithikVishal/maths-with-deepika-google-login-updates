...
export function LoginForm({
  ...
  const loginTitle = loginKind === "admin" ? "Admin Login" : "Student Login";
  ...
  return (
    <div className={loginKind === "admin" ? "mx-auto max-w-xl" : "grid gap-6 md:grid-cols-2"}>
      <form action={loginFormAction} className="rounded-soft bg-white p-6 shadow-soft">
        ...
        <input type="hidden" name="expected_role" value={loginKind === "admin" ? "admin" : "student"} />
      ...
      {loginKind === "admin" ? null : mode === "login" ? (
        <div className="rounded-soft bg-white p-6 shadow-soft">
          ...
          <p className="mt-4 leading-7 text-ink/70">
            Students can create an account and start learning. Existing students can use the login details shared by the admin.
          </p>
