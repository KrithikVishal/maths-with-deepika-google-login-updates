...
export function NotificationPanel({
  ...
  return (
    <div className="rounded-[2rem] bg-white p-5 shadow-soft ring-1 ring-borderSoft">
      ...
      {notifications.length === 0 ? (
        ...
        <div className="grid gap-3">
          {notifications.map((notification) => (
            <div key={notification.id} className={`rounded-2xl border p-4 ${notification.read ? "border-borderSoft bg-beige/40" : "border-coral/20 bg-coral/10"}`}>
              <div className="flex items-start justify-between gap-3">
                ...
                {!notification.read ? (
                  <form action={markNotificationReadAction}>
                    ...
                    <button className="focus-ring grid h-9 w-9 place-items-center rounded-full bg-white text-blueDeep shadow-sm ring-1 ring-borderSoft" title="Mark as read">
